...
-record(image,
	{
	 name,        %% name of image
	 path,        %% internal path of image
	 primary,     %% primary backup server/site
	 secondary,   %% secondary backup server/site
	 bmap,        %% bitmap file name
	 blk,         %% name of file containing checks sum and md5 hashes
	 block_size,  %% block size, default 4k
	 size         %% image size (max) in bytes
	}).
...
%%-define(verbose(F,A), io:format("~s:~w: "++(F)++"\n",[?MODULE,?LINE|(A)])).
...
init(Args) ->
    ?debug("args = ~p", [Args]),
    Images      = proplists:get_value(images, Args, []),
...
fsync(_Ctx, _Inode, IsDataSync, Fi, _Cont, State) ->
    ?debug("fsync(~p)", [{_Inode,IsDataSync,Fi}]),
...
lookup(_Ctx, Parent, BinName, _Cont, State) ->
    Name = to_list(BinName),
    ?debug("lookup(~p)", [{Parent,Name}]),
    case lookup_path_name(Parent, State) of
	false -> 
	    { #fuse_reply_err{ err = enoent }, State };
	"" ->
	    case lists:keymember(Name, #image.name, State#state.images) of
		true ->
		    RealName = filename:join(State#state.root,Name),
		    case read_stat_(RealName,0) of
			{ok,A} ->
...
read(_Ctx, _Inode, Size, Offset, Fi, _, State) ->
    ?debug("read(~p)", [{_Inode,Size,Offset,Fi}]),
    ?verbose("read: offset:~w size:~w\n", [Offset, Size]),
    case fh_open(Fi#fuse_file_info.fh, State) of
	{ok,{Fd,_Bmap,State1}} ->
	    case file:pread(Fd, Offset, Size) of
		{ok,Data} ->
		    case State#state.delay_read of
			0 -> ok;
			Delay -> receive after Delay -> ok end
		    end,
...
readdir (_, Inode, Size, Offset, _Fi, _, State) ->
	...
	false ->
	    { #fuse_reply_err{ err = enoent }, State };
	RealDirName when RealDirName =:= State#state.root ->
	    DirList = [Name || #image{ name = Name } <- State#state.images],
	    List = dir_list_(Inode,RealDirName,DirList,
			     -Offset,Size,State,[]),
	    { #fuse_reply_direntrylist { direntrylist = List }, State};
	_RealDirName ->
	    ?debug("realdirname = ~p\n", [_RealDirName]),
	    { #fuse_reply_direntrylist { direntrylist = [] }, State}
    end.
	    
	    %% ?debug("realdirname = ~p\n", [RealDirName]),
	    %% case file:list_dir(RealDirName) of
	    %% 	{ok,DirList} ->
	    %% 	    DirList1 = [File || File <- DirList, 
	    %% 				not lists:suffix(".bmap", File)],
	    %% 	    List = dir_list_(Inode,RealDirName,DirList1,
	    %% 			     -Offset,Size,State,[]),
	    %% 	    { #fuse_reply_direntrylist { direntrylist = List }, State};
	    %% 	{error, Err} ->
	    %% 	    { #fuse_reply_err{ err = Err }, State }
...
setattr(_Ctx, Inode, Attr, ToSet, Fi, _Cont, State) ->
	...
	RealName ->
		...
		if is_record(Fi, fuse_file_info) ->
			case fh_open(Fi#fuse_file_info.fh, State) of
			    {ok,{Fd0,_Bmap,State0}} -> {Fd0,State0};
			    _Error -> {null,State}
			end;
		   true -> { null, State}
		end,
...
write(_Ctx, _Inode, Data, Offset, Fi, _Cont, State) ->
    ...
    Sz = erlang:iolist_size(Data),
    ?verbose("write: offset:~w size:~w\n", [Offset, Sz]),
    case fh_open(Fi#fuse_file_info.fh, State) of
	{ok,{Fd,Bmap,State1}} ->
	    ok = fuserl_image_bmap:mark(Bmap, Offset, Sz),
	    case file:pwrite(Fd, Offset, Data) of
		ok ->
		    case State#state.delay_write of
			0 -> ok;
			Delay -> receive after Delay -> ok end
		    end,		    
...
ensure_images(_RootDir,[]) ->
    [];
ensure_images(RootDir, [Opts|Images]) ->
    Name = proplists:get_value(name, Opts),
    Primary = proplists:get_value(primary, Opts, undefined),
    Secondary = proplists:get_value(secondary, Opts, undefined),
    Bmapname = proplists:get_value(bmap, Opts, Name++".bmap"),
    Blockname = proplists:get_value(blk, Opts, Name++".blk"),
    BlockSize = parse_size(proplists:get_value(block_size, Opts), 4096),
    Size  = parse_size(proplists:get_value(size, Opts), 0),
    Path = filename:join(RootDir, Name),
    %% make sure bitmap file is created
    try fuserl_image_bmap:create(filename:join(RootDir, Bmapname),
				 BlockSize, Size) of
	Bmap -> fuserl_image_bmap:close(Bmap)
    catch
	error:file_exist ->
	    ok
    end,
    case read_stat_(Path, 0) of
	{ok, Stat} ->
	    Size1 = if Size =:= 0 -> Stat#stat.st_size; true -> Size end,
	    [#image { name = Name,
		      path = Path,
		      primary = Primary,
		      secondary = Secondary,
		      ...
		      size = Size1 } | ensure_images(RootDir,Images)];
	{error, enoent} ->
	    %% create image file
	    ok = file:write_file(Path, <<>>),
	    {ok, _Stat} = read_stat_(Path, 0),
	    [#image { name = Name,
		      path = Path,
		      primary = Primary,
		      secondary = Secondary,
		      ...
		      blk  = Blockname,
		      block_size = BlockSize,
		      size = Size } | ensure_images(RootDir, Images)]
    end.


parse_size(Size) -> parse_size(Size, undefined).

parse_size(0, Default) -> Default;
parse_size(undefined, Default) -> Default;
parse_size(Size, _Default) when is_integer(Size), Size > 0 ->
    Size;
parse_size("", Default) -> Default;
parse_size(String, _Default) when is_list(String) ->
    parse_size_(String, 0).

parse_size_("", Sum) -> Sum;	
parse_size_(String,Sum) ->    
    case string:to_integer(String) of
	{A, [$K|String1]} -> parse_size_(String1, A*1024+Sum);
	{A, [$M|String1]} -> parse_size_(String1, A*(1024*1024)+Sum);
	{A, [$G|String1]} -> parse_size_(String1, A*(1024*1024*1024)+Sum);
	{A, []} -> A+Sum
    end.
...
fh_close(Fh, State) ->
	...
	[E] ->
	    ...
	    fuserl_image_bmap:close(E#fh_ent.bmap),
...
fh_open(Fh, State) ->
	...
	[E] ->
	    fh_tick(Fh, State),
	    {ok, {E#fh_ent.fd,E#fh_ent.bmap,State}};
	[] ->
...
fh_open_name(Filename, E, State) ->
	...
	{ok,Fd} ->
	    Bmap = case E#fh_ent.bmap of
		       undefined ->
			   fuserl_image_bmap:open(Filename++".bmap");
		       Bmap0 ->
			   fuserl_image_bmap:reopen(Filename++".bmap", Bmap0)
		   end,
	    Fh = E#fh_ent.fh,
	    ...
	    E1 = E#fh_ent { fd = Fd, bmap=Bmap, time = Time0 },
...
write_stat_(Filename, Stat, Mask, Fd) ->
    ...
    if Fd =/= null, is_integer(FI#file_info.size) ->
	    _R1 = file:position(Fd, FI#file_info.size),
	    R2 = file:truncate(Fd),
	    ?debug("set_size ~w = ~p,~p", [FI#file_info.size,_R1,R2]),
	    R2;
       true ->
	    ok
    end,
